            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (SwingUtilities.isLeftMouseButton(e) && e.getClickCount() > 1) {
                    if (!searchBoxHelper.getText().isEmpty()) {
                        try {
                            searchBoxHelper.setText(searchBoxHelper.getText() + " && " + fofaLineTableModel.getColumnName(column) + "=\"" + fofaLineTableModel.getLineEntries().get(rows[0]).fetchValue(getColumnName(column)) + "\"");
                        } catch (Exception ex) {
                           ex.printStackTrace(stderr);
                        }
                    } else {
                        try {
                            searchBoxHelper.setText(fofaLineTableModel.getColumnName(column) + "=\"" + fofaLineTableModel.getLineEntries().get(rows[0]).fetchValue(getColumnName(column)) + "\"");
                        } catch (Exception ex) {
                            ex.printStackTrace(stderr);
                        }
                    }
