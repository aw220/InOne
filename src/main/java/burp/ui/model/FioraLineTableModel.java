...
import burp.action.IntArraySlice;
...
public class FioraLineTableModel extends AbstractTableModel implements Serializable {
	...
	private FioraLineEntry currentlyDisplayedItem;
	 ...
	 */
	private IndexedLinkedHashMap<String, FioraLineEntry> lineEntries =new IndexedLinkedHashMap<String, FioraLineEntry>();
	...
	private static List<String> titletList = FioraLineEntry.fetchTableHeaderList();//还是需要删减字段
	...
	public IndexedLinkedHashMap<String, FioraLineEntry> getLineEntries() {
		return lineEntries;
	}

	public void setLineEntries(IndexedLinkedHashMap<String, FioraLineEntry> lineEntries) {
	...
	{
		FioraLineEntry entry = lineEntries.getValueAtIndex(rowIndex);
	...
	public void clear(boolean syncToFile) {
		...
		//虽然触发了，却无法更新数据库。
		this.setLineEntries(new IndexedLinkedHashMap<String, FioraLineEntry>());//如果ListenerIsOn，将会触发listener
	...
	public void addNewLineEntry(FioraLineEntry lineEntry){
		if (lineEntry == null) {
			return;
		}
	 ...
	 */
	public FioraLineEntry findLineEntry(String url) {//这里的URL需要包含默认端口!!!
	 ...
	 */
	@Deprecated
	public List<FioraLineEntry> findLineEntriesByHost(String host) {//
		if (lineEntries == null) return null;
		List<FioraLineEntry> result = new ArrayList<FioraLineEntry>();
	...
	public FioraLineEntry getCurrentlyDisplayedItem() {
		return this.currentlyDisplayedItem;
	}

	public void setCurrentlyDisplayedItem(FioraLineEntry currentlyDisplayedItem) {
