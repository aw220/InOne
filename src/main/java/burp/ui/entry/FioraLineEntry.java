...
public class FioraLineEntry{
	private static final Logger log=LogManager.getLogger(FioraLineEntry.class);
	...
	public static FioraLineEntry FromJson(String json){//注意函数名称，如果是get set开头，会被认为是Getter和Setter函数，会在序列化过程中被调用。
		return new Gson().fromJson(json, FioraLineEntry.class);
	}
	
	public static List<String> fetchFieldNames(){
		Field[] fields = FioraLineEntry.class.getDeclaredFields();
	...
	public Object callGetter(String paraName) throws Exception {
		Method[] methods = FioraLineEntry.class.getMethods();
	...
	public Object fetchValue(String paraName) throws Exception {
		...
		Field field = FioraLineEntry.class.getDeclaredField(paraName);
